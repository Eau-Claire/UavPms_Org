...
- [ ] 27b. **API lấy danh sách người dùng cho phân công chuyến bay (`GetAssignableUsersQuery` / `GET /api/v1/users/assignable`)**: Yêu cầu quyền quản trị (`SystemAdmin`/`Manager`), chỉ trả về người dùng hoạt động (`Status == "Active"`) có vai trò `Inspector`, không bao gồm thông tin nhạy cảm.
