...
def test_upload_inspection_image_request_is_handled(
    ...
    response = client.post("/api/v1/inspections/upload", data=form, files=files, headers=role_headers("inspector"))
...
def test_get_inspection_report_fake_id_is_handled(
    ...
    response = client.get(f"/api/v1/inspections/report/{fake_id}", headers=role_headers("inspector"))
